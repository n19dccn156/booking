...
// ON rt.id = r.id and is_active = true
    ...
    @Query(value = ""+
    "SELECT "+
        "rt.id, rt.name, rt.avatar, rt.price, (rt.quantity-r.quantity) quantity, is_active, hotels_id "+
    "FROM room_type rt "+
    "INNER JOIN ( "+
        "SELECT "+
            "room_type_id id, SUM(quantity) quantity "+
            "FROM hotel_order_details "+
        "WHERE "+ 
            "hotel_orders_id IN ("+
            "SELECT id FROM hotel_orders "+
            ...
            "(checkout > ?2 and checkout <= ?3)) " +
        "GROUP BY room_type_id) r "+
    "ON rt.id = r.id and is_active = true", nativeQuery = true)
